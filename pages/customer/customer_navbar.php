<nav class="navbar">
    <div class="container">
        <!-- Branding -->
        <a href="customer_dashboard.php" class="navbar-brand">
           
        </a>

        <!-- Menu Items -->
        <ul class="navbar-menu">
            <li><a href="customer_dashboard.php" class="active"><i class="fas fa-home"></i> Dashboard</a></li>
            <li><a href="customer_profile.php"><i class="fas fa-user"></i> Profile</a></li>
            <li><a href="customer_recommendations.php"><i class="fas fa-lightbulb"></i> Recommendations</a></li>
            <li><a href="customer_appointments.php"><i class="fas fa-calendar-alt"></i> Appointments</a></li>
            <li><a href="customer_notifications.php"><i class="fas fa-bell"></i> Notifications</a></li>
            <li><a href="/fyp_kusma/pages/login/login.php" class="logout-link"><i class="fas fa-sign-out-alt"></i> Logout</a></li>
        </ul>
    </div>
</nav>
